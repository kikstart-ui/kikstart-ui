...
@Component({
  ...
  template: `
    ...
    <ng-container *ngIf="isAction">
      <button [disabled]="disabled || button?.disabled" [class]="btnClass" (click)="emitAction(button, payload)">
        <ui-loading-icon *ngIf="loading" [class.mr-2]="loadingClass" [icon]="loadingIcon"> </ui-loading-icon>
        <ui-label [label]="button?.label" [icon]="button?.icon" [iconAfter]="button?.iconAfter"></ui-label>
      </button>
    </ng-container>
    <ng-container *ngIf="isHandler">
      <button [disabled]="disabled || button?.disabled" [class]="btnClass" (click)="emitHandler(button, payload)">
        <ui-loading-icon *ngIf="loading" [class.mr-2]="loadingClass" [icon]="loadingIcon"> </ui-loading-icon>
        <ui-label [label]="button?.label" [icon]="button?.icon" [iconAfter]="button?.iconAfter"></ui-label>
      </button>
    </ng-container>
...
export class UiButtonComponent {
  ...
  get btnClass() {
    return [
      'btn',
      this.button.className ? this.button.className : this.buttonClass ? this.buttonClass : 'btn-outline-info',
    ].join(' ')
  }
  get isAction() {
